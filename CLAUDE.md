...
- **Amazon pricing display** (owner 2026-07): NEVER show a price, saving, discount or deal that did not come from the PA-API within the last **24 hours** — that is the Associates Operating Agreement, and it covers promotions, not just prices. Enforced structurally, not by convention: `AmazonProduct.pricing` is populated in exactly one place (`mapItems()`, off a live API response) and `stripPricing()` removes it on the ≤7d stale-while-error cache and on any catalog snapshot older than 24h. The static house catalog carries **no prices at all** — do not re-add a `price` field to it. `newsromania-amazon-catalog.timer` (daily, all marketplaces) refreshes the pool via the PA-API when the account is eligible, and meanwhile link-checks the static ASINs and prunes dead ones. Never re-harvest prices by scraping product pages.
