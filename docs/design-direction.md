...
The logo decides the direction: `assets/logo-full.png` is a distressed **serif** wordmark with a black baseline rule — a newspaper masthead, not a tech logo — built for light ground. „Broadsheet Tricolor” is therefore the base: a warm paper page, near-black serif headlines, black masthead rules, and the tricolor ring flattened into a thin, disciplined accent system. This register signals *trust* the way a quality Romanian broadsheet does, while the layout system (sticky CSS-only nav, list-not-cards feed, zero-CLS ads, AA-verified palette) keeps it modern and mobile-first.
