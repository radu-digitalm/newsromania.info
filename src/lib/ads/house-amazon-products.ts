 ...
 *
 * ⚠️ NO PRICES HERE, BY DESIGN (owner, iulie 2026). This file is a static
 * snapshot; nothing refreshes it. Amazon only permits displaying price / savings
 * / promotion data that came from the Product Advertising API and is under 24h
 * old, so these entries deliberately carry no `pricing` — the card renders
 * title + image + CTA. Prices reappear on their own once the PA-API is eligible
 * and the daily worker publishes a fresh snapshot (see amazon-catalog.ts).
 * Do not re-add a `price` field here, however tempting.
 *
 * Dead ASINs are pruned at render time from the daily link-check's dead set
 * (scripts/worker/amazon-catalog.mjs), not by editing this file.
 */
